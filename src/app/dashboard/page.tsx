...
export default function DashboardAtletas() {
  ...
  if (loading) return <div className="p-6 text-center text-blue-900 font-black italic uppercase animate-pulse">A carregar o Dojô...</div>;
  ...
  return (
    <div className="p-4 md:p-8 bg-gray-100 min-h-screen relative text-black text-left">
      ...
      {/* CARDS DE RESUMO (6 COLUNAS) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 mb-8">
        <div className="bg-white p-4 rounded-xl shadow-sm border-b-4 border-blue-600">
          <p className="text-gray-400 text-[9px] font-black uppercase tracking-widest">Total Geral</p>
          <h2 className="text-2xl font-black text-blue-900">{atletas.length}</h2>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border-b-4 border-green-500">
          <p className="text-gray-400 text-[9px] font-black uppercase tracking-widest">No Tatame</p>
          <h2 className="text-2xl font-black text-green-600">{atletas.filter(a => a.ativo !== false).length}</h2>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border-b-4 border-emerald-500">
          <p className="text-gray-400 text-[9px] font-black uppercase tracking-widest">Em Dia</p>
          <h2 className="text-2xl font-black text-emerald-600">{atletas.filter(a => a.statusPagamento === 'EM_DIA').length}</h2>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border-b-4 border-red-500">
          <p className="text-gray-400 text-[9px] font-black uppercase tracking-widest">Pendentes</p>
          <h2 className="text-2xl font-black text-red-600">{atletas.filter(a => a.statusPagamento === 'PENDENTE').length}</h2>
        </div>
        <div className="lg:col-span-2 bg-blue-900 p-4 rounded-xl shadow-lg flex flex-col justify-center gap-2">
          <button onClick={() => setIsModalOpen(true)} className="w-full bg-white text-blue-900 text-[9px] font-black uppercase py-2 rounded shadow-sm hover:bg-blue-50 transition-all">+ Novo Aluno</button>
          <button onClick={() => setIsModalOpen(true)} className="w-full bg-blue-700 text-white text-[9px] font-black uppercase py-2 rounded hover:bg-blue-600 transition-all">+ Novo Professor</button>
        </div>
      </div>

      {/* TABELA COM TUDO: VENCIMENTO + STATUS + BOTÕES */}
      <div className="bg-white rounded-xl shadow-xl overflow-hidden border border-gray-200">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#1e3a8a] text-white">
              <tr>
                <th className="p-4 font-bold uppercase text-[11px] tracking-widest">Atleta / Info</th>
                <th className="p-4 font-bold uppercase text-[11px] tracking-widest text-center">Vencimento</th>
                <th className="p-4 font-bold uppercase text-[11px] tracking-widest text-center">Pagamento</th>
              ...
              {atletas.map((atleta) => {
                ...
                return (
                  <tr key={atleta.id} className="hover:bg-blue-50/50 transition-colors">
                    <td className="p-4">
                      <div className="font-extrabold text-gray-800 text-sm uppercase mb-1">
                        {atleta.nomeCompleto || atleta.nome || "---"}
                      </div>
                      <div className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">
                        {atleta.turno || "N/A"} • {atleta.graduacao || "BRANCA"}
                      </div>
                    </td>
                    <td className="p-4 text-center font-bold text-gray-600 text-xs">
                      Dia {atleta.diaVencimento || 28}
                    </td>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => gerarDocumentoAtleta(atleta)} className="p-2 bg-blue-600 text-white rounded shadow-sm hover:bg-blue-700 active:scale-90" title="Financeiro">💰</button>
                        <button onClick={() => downloadRelatorioTecnico(atleta.id, atleta.nomeCompleto)} className="p-2 bg-slate-800 text-white rounded shadow-sm hover:bg-black active:scale-90" title="Técnico">🥋</button>
                        <button 
                          onClick={() => handleToggleStatus(atleta.id, atleta.ativo)}
                          className={`px-3 py-1 rounded shadow-sm text-[10px] font-black text-white active:scale-90 ${statusAtivo ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'}`}
                        >
                          {statusAtivo ? "OFF" : "ON"}
                        </button>
                      </div>
